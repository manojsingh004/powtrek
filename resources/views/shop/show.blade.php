...
@section('description_custom')
    {{$product->description ? $product->description : trans('seo.description')}}
@endsection
                    ...
                    <div class="card rounded-large shadow-large card-border-0">
                        ...
                        <div class="card-body">
                            ...
                            @if($product->url_video)
                                ...
                                <button type="button" class="btn btn-1 btn-warning btn-block mt-4 text-white"
                                        data-toggle="modal" data-target="#product{{$product->id}}">
                                    PREVIEW
                                </button>
                                        ...
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title"
                                                    id="product{{$product->id}}Label">{{$product->name}}</h5>
                                            ...
                                            <div class="modal-body">
                                                <iframe width="800" class="yvideo" height="400"
                                                        src="{{$product->url_video}}" title="YouTube video player"
                                                        frameborder="0"
                                                        allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                                                        allowfullscreen></iframe>
                                            </div>
                                            <div class="modal-footer">
                                                <button id="fecharvideo" type="button" class="btn btn-secondary"
                                                ...
                                                @if (auth()->check()
                                                    ...
                                                    <button class="btn btn-1 btn-primary"
                                                            @if ($product->quantity == 0 && $product->type == 'physical') disabled
                                                            @endif type="button" data-toggle="modal"
                                                            @auth data-target="#buyNowForm"
                                                            @else data-target="#loginFormModal" @endauth>
                                                ...
                                                @elseif (auth()->check() && auth()->id() == $product->user()->id)
                                                    <a class="btn btn-1 btn-primary" href="#" data-toggle="modal"
                                                    ...
                                                    <form method="post"
                                                        ...
                                                        @csrf
                                                        <button class="btn btn-1 btn-outline-danger actionDeleteItem"
                                                                type="button">
                            ...
                            @if (auth()->check()
                                ...
                                <button class="btn btn-1 btn-primary btn-block mt-4"
                                        @if ($product->quantity == 0 && $product->type == 'physical') disabled
                                        @endif type="button" data-toggle="modal" @auth data-target="#buyNowForm"
                                        @else data-target="#loginFormModal" @endauth>
                                    {{ $product->quantity == 0 && $product->type == 'physical' ? __('general.sold_out') : __('general.buy_now') }}
                                </button>
                            ...
                            @elseif (auth()->check() && auth()->id() == $product->user()->id)
                                ...
                                <form method="post" action="{{ url('delete/product', $product->id) }}">
                                    @csrf
                                    <button class="btn btn-1 btn-outline-danger btn-block mt-2 actionDeleteItem"
                                            type="button">
                                        <i class="bi-trash mr-1"></i> {{ __('admin.delete') }}
                                    </button>
                                </form>
                            ...
                            <div class="w-100 d-block mt-3">
                                <i class="bi-cart2 mr-2"></i> {{ __('general.purchases') }}
                            ...
                            @if ($product->type == 'digital')
                                ...
                                <div class="w-100 d-block mt-3">
                                    <i class="bi-box-seam mr-2"></i> {{ __('general.file') }} <span
                            ...
                            @elseif ($product->type == 'custom')
                                <div class="w-100 d-block mt-4">
                                    <i class="fa fa-fire-alt mr-2"></i> {{ __('general.delivery_time') }}
                                    ({{$product->delivery_time}} {{ trans_choice('general.days', $product->delivery_time) }}
                                    )
                                </div>
                            ...
                            @else
                                ...
                                @if ($product->quantity <> 0)
                                    <div class="w-100 d-block mt-4">
                                        <i class="bi-boxes mr-2"></i> {{ __('general.quantity') }} <span
                                                class="badge badge-pill badge-success">{{ $product->quantity }}</span>
                                    </div>
                                @else
                                ...
                                @if ($product->shipping_fee <> 0.00)
                                    <div class="w-100 d-block mt-4">
                                        <i class="bi-truck mr-2"></i> {{ __('general.shipping_fee') }}
                                        - {{ Helper::amountFormatDecimal($product->shipping_fee) }}
                                        <small>{{ $settings->currency_code }}</small>

                                        @if ($product->country_free_shipping)
                                            <small><em>({{ __('general.free_shipping') }} {{ $product->country()->country_name }}
                                                    )</em></small>
                                        @endif
                                    </div>
                            ...
                            <div class="w-100 d-block mt-4">
                                <i class="feather icon-share mr-2"></i> <span
                                        class="mr-2">{{ __('general.share') }}</span>
                            ...
                            @if (auth()->check() && auth()->id() != $product->user()->id)
                                <div class="w-100 d-block mt-4">
                                    <button type="button" class="btn e-none btn-link text-danger p-0"
                                            data-toggle="modal" data-target="#reportItem">
                        ...
                        <form method="POST" action="{{url('report/item', $product->id)}}" enctype="multipart/form-data">
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="submit" class="btn btn-xs btn-white sendReport ml-auto">
                                    <i></i> {{trans('general.report_item')}}</button>
                            </div>
                        </form>
                    </div><!-- Modal content -->
                </div><!-- Modal dialog -->
