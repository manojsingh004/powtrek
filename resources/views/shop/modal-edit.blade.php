            ...
            <div class="modal-body p-0">
                <div class="card bg-white shadow border-0">
                    ...
                    <div class="card-body px-lg-5 py-lg-5 position-relative">
                        ...
                        <div class="mb-4">
                            {{ __('admin.edit') }} <i class="bi-arrow-right ml-1 mr-1"></i>
                        ...
                        <form method="post" action="{{url('edit/product', $product->id)}}" id="shopProductForm">
                            ...
                            <div class="form-group">
                                <input type="text" class="form-control isNumber" value="{{ $product->price }}"
                                       autocomplete="off" name="price" placeholder="{{ __('general.price') }}">
                            </div>

                            @if ($product->type == 'physical')
                                <div class="form-group">
                                    <input type="text" class="form-control isNumber"
                                           value="{{ $product->shipping_fee }}" autocomplete="off" name="shipping_fee"
                                           placeholder="{{ __('general.shipping_fee') }}">
                                </div>
                                ...
                                <div class="form-group">
                                    <select name="country_free_shipping" class="form-control custom-select">
                                        ...
                                        @foreach (Countries::orderBy('country_name')->get() as $country)
                                            <option
                                                @if ($product->country_free_shipping == $country->id ) selected="selected"
                                    ...
                                    <select name="delivery_time" class="form-control custom-select">
                                        ...
                                        @for ($i=1; $i <= 30; ++$i)
                                            <option @if ($product->delivery_time == $i) selected
                                                    @endif value="{{$i}}">{{$i}} {{ trans_choice('general.days', $i) }}</option>
                                        @endfor
                                    </select>
                                </div>
                            @endif
                            ...
                            @if ($product->type == 'physical')
                                ...
                                <div class="form-group">
                                    <input type="text" class="form-control" value="{{ $product->box_contents }}"
                                           name="box_contents" placeholder="{{ __('general.box_contents') }}">
                                </div>
                            @endif
                            ...
                            <div class="form-group">
                                <select name="category" class="form-control custom-select">
                                    ...
                                    @foreach (App\Models\ShopCategories::orderBy('name')->get() as $category)
                                        <option @if ($product->category == $category->id) selected
                            ...
                            <div class="form-group">
                                <input type="text" class="form-control" name="url_video"
                                       value="{{ $product->url_video }}" placeholder="Add a youtube video URL">
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                <div class="custom-control custom-switch custom-switch-lg">
                                    <input type="checkbox" class="custom-control-input" name="status" value="1"
                                           @if ($product->status) checked @endif id="customSwitch2">
                                    <label class="custom-control-label switch"
                                           for="customSwitch2">{{ __('general.status') }}</label>
                                </div>
                            </div>
