...
trait Functions {
	...
	// Insert Deposit Pix Method (Add funds user wallet)
	public function deposit_pix($userId, $txnId, $amount, $paymentGateway, $taxes, $screenshotTransfer = '',$payer = '')
	{
        $payment = PaymentGateways::whereName($paymentGateway)->firstOrFail();
//        dd($payment);
		...
		$sql = new Deposits();
		$sql->user_id = $userId;
		$sql->payer_name = $payer;
		$sql->amount = $amount;
		$sql->payment_gateway = $paymentGateway;
		$sql->status = $paymentGateway == 'Pix' ? 'pending' : 'active';
        $sql->screenshot_transfer = $screenshotTransfer;
		$sql->percentage_applied = $percentageApplied;
		$sql->transaction_fee = $transactionFee;
		$sql->taxes = $taxes;
