...
class AdminController extends Controller
{
	...
	public function deposits_pix()
    {
        $data = Deposits::orderBy('id', 'desc')->where('payment_gateway', 'Pix')->paginate(30);
        return view('admin.deposits-pix')->withData($data);
    }//<--- End Method

    public function depositsViewPix($id)
    {
        $data = Deposits::findOrFail($id);
        return view('admin.deposits-view-pix')->withData($data);
    }//<--- End Method

    public function approveDeposits_Pix(Request $request)
    {

        $sql = Deposits::find($request->id);
        $sql->update(['amount'=>$request->amount_pix_added]);
        ...
        return redirect('panel/admin/deposits-pix');
    }//<--- END METHOD
