...
class AddFundsController extends Controller
{
    ...
    public function send()
    {
        ...
        $messages = array(
            ...
            'image_pix.required_if' => trans('general.please_select_image'),
            'pix_trx_id.required_if' => trans('general.transaction_id_error'),
            'pix_payer.required_if' => trans('general.payer_name_error'),
        ...
        $validator = Validator::make($this->request->all(), [
            ...
            'image_pix' => 'required_if:payment_gateway,==,Pix|mimes:jpg,gif,png,jpe,jpeg|max:' . $this->settings->file_size_allowed_verify_account . '',
            'pix_payer' => 'required_if:payment_gateway,==,Pix'
        ], $messages);
 ...
 public function sendPixTransfer()
  {

    $path = config('path.admin');

    if ($this->request->hasFile('image_pix')) {
    $extension = $this->request->file('image_pix')->getClientOriginalExtension();
    $fileImage = 'bt_'.strtolower(auth()->user()->id.time().str_random(40).'.'.$extension);

    $this->request->file('image_pix')->storePubliclyAs($path, $fileImage);
      $pix_payer = $this->request->post('pix_payer');
   }//<====== End HasFile

   // Insert Deposit

   $deposit = $this->deposit_pix(
      auth()->user()->id,
	   'bt_'.str_random(25),
      $this->request->amount,
      'Pix',
    ...
    public function sendFlutterwave()
    {
        try {
            ...
            $data = [
                'payment_options' => 'card,banktransfer,pix,mobilemoneyghana,mpesa',
                'amount' => $amountFixed,
                'email' => request()->email,
                'tx_ref' => $reference,
                'currency' => $this->settings->currency_code,
