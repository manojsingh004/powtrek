		   	...
		   	if(window.File && window.FileReader && window.FileList && window.Blob) {
		        //check empty input filed
		         ...
		         if(ftype == 'video/mp4' || ftype == 'video/quicktime') {
				   ...
				   $('#previewImagePix').html('<i class="fa fa-play-circle text-info mr-1"></i> '+$fileName);
		           $('#removePhoto').show();
		         }
		         ...
		         if(ftype == 'audio/mpeg') {
				   ...
				   $('#previewImagePix').html('<i class="fa fa-music text-info mr-1"></i> '+$fileName);
		           $('#removePhoto').show();
		         }
		               ...
		               if(image.height > image.width) {
		                 ...
		                 var $imageWidth = 65;
							...
							$('#previewImagePix').html('<img src="'+e.target.result+'" class="rounded" width="'+$imageWidth+'" />');
		               $('#removePhoto').show();
           ...
           cancelButtonText: cancel_confirm,
            ...
            function(isConfirm){
			 ...
			 $(".fileVerifiyAccount").on('change', function() {
				 ...
				 _this.parents('.previewImageVerification').find('.previewImagePix').html('');
			      ...
			      //check empty input filed
			       ...
			       if(!rFilter.test(oFile.type)) {
			         ...
			         $(this).val('');
						...
						_this.parents('.previewImageVerification').find('.previewImagePix').html('<i class="fas fa-image text-info mr-1"></i> <strong>' + oFile.name + '</strong>');
			 		}
			 	} else{
...
$("#fileZip").on('change', function() {
 ...
 if(window.File && window.FileReader && window.FileList && window.Blob) {
	 ...
	 if($(this).val()) {
		 ...
		 $('#previewImagePix').html('<i class="fa fa-paperclip text-info"></i> <strong>' + oFile.name + '</strong>');
		 $('#removePhoto').show();
...
$(document).on('click','#restrictUser',function(e) {
	...
	$(document).on('click', '#click_copy_btn_pix', function() {

	  var copyText = $("#pix_unique_id").val();
	  $('.copied_text_message_d').css('display','block');
	  ...
	  function copyToClipboard(text) {
       ...
       setTimeout(function(){$('.copied_text_message_d').css('display','none')}, 1500);
     }
