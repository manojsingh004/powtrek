...
(function ($) {
    ...
    $('button[id=fecharvideo]').on('click', function (e) {
        console.log('yvideo')
                    ...
                    if (result.error) {
                        ...
                        $.ajax({
                            ...
                            success: function (result) {
                                ...
                                if (result.success) {
                                    swal({
                                        title: thanks, text: tip_sent_success, type: "success", confirmButtonText: ok
                                    });
                                    $('#tipForm').modal('hide');
                                    $('.InputElement').val('');
                                    $('#tipBtn').removeAttr('disabled');
...
//============ End Payment =================//
    ...
    $('#tipForm').on('show.bs.modal', function (event) {
        ...
        var modal = $(this);
        modal.find('.card-header').css({
            height: '100px',
            background: color_default + ' ' + _coverUser + ' no-repeat center center',
        ...
        $(tipAmount).on('keyup', function () {
            ...
            if (valueOriginal.length == 0 || valueOriginal == '' || amount < minTipAmount || amount > maxTipAmount) {
                // Reset
